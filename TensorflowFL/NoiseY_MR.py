...
def getParmsAndLearningRate(agent_no):
    f = open(os.path.join(os.path.dirname(__file__), "Plus_weights_" + str(agent_no) + ".txt"))
    content = f.read()
    g_ = content.split("***\n--------------------------------------------------")
    parm_local = []
    learning_rate_list = []
    ...
    f = open(os.path.join(os.path.dirname(__file__), "Plus_bias_" + str(agent_no) + ".txt"))
    content = f.read()
    g_ = content.split("***\n--------------------------------------------------")
    bias_local = []
    for j in range(len(g_) - 1):
...
if __name__ == "__main__":
    ...
    for index in range(NUM_AGENT):
        f = open(os.path.join(os.path.dirname(__file__), "Plus_weights_" + str(index) + ".txt"), "w")
        f.close()
        f = open(os.path.join(os.path.dirname(__file__), "Plus_bias_" + str(index) + ".txt"), "w")
        f.close()
    ...
    for round_num in range(50):
        ...
        for local_index in range(len(local_models)):
            f = open(os.path.join(os.path.dirname(__file__), "Plus_weights_" + str(local_index) + ".txt"), "a", encoding="utf-8")
            ...
            print("-" * 50, file=f)
            f.close()
            f = open(os.path.join(os.path.dirname(__file__), "Plus_bias_" + str(local_index) + ".txt"), "a", encoding="utf-8")
            line = ""
