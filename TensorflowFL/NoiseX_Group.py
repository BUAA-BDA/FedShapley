...
if __name__ == "__main__":
    ...
    #
    # with open("random_index.txt", "r") as randomIndex:
    #     lines = randomIndex.readlines()
    #     for line in lines:
    #         rand_index.append(eval(line))
    # with open("random_label.txt", "r") as randomLabel:
    ...
    while(isNotConverge(last_u,U) and round_t<CONVERGE_UPPER):
        ...
        u_t=trainGroup(random_seq,federated_train_data_divide,test_images,test_labels_onehot)
    ...
    U_D=trainGroup(range(0,NUM_AGENT),federated_train_data_divide,test_images,test_labels_onehot)
    agent_shapley=solveFeasible(NUM_AGENT,U_D,U)
    print(agent_shapley)
    print("end_time", time.time() - start_time)

    #f_ini_p = open(os.path.join(os.path.dirname(__file__), "initial_model_parameters.txt"), "r")
    with open(os.path.join(os.path.dirname(__file__), "log_Noise_Group"),"wb") as f:
        pickle.dump(U_D,f)
        pickle.dump(rec_u,f)