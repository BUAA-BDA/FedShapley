...
def ModelValuation(train_set_index, distri_type, data_agt):
    federated_train_data = []
    data_num_sum = 0
    for index in train_set_index:
        federated_train_data.append(federated_train_data_divide[index])
        data_num_sum += data_agt[index]

    agents_weights = []
    for item in train_set_index:
        agents_weights.append(data_agt[item] / data_num_sum)
...
if __name__ == "__main__":
    ...
    all_in_performance = ModelValuation(initial_permutation, DISTRIBUTION_TYPE,data_num)
    empty_performance = ModelValuation([], DISTRIBUTION_TYPE,data_num)
        ...
        for j in range(1, NUM_AGENT+1):
            ...
            if abs( all_in_performance - v[j-1]) < performance_tolerence:
                ...
                if getCachedPerformance(l) == -1:
                    v[j] = ModelValuation(l, DISTRIBUTION_TYPE, data_num)
